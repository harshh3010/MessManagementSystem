...
exports.signUp = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    await sendMail({
      email: newUser.email,
      subject: "Mess management app email verification",
      message: message,
    });
    ...
    res.status(200).json({
      status: "success",
      message:
        "You have been registered successfully. Please verify your email (within 10 minutes) to continue.",
    });
  } catch (err) {
...
exports.login = catchAsync(async (req, res, next) => {
  ...
  if (!user.isVerified) {
    ...
    return next(
      new AppError(
        "Account not verified. Please verify your email to proceed further.",
        401
      )
    );
  } else {
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    await sendMail({
      email: user.email,
      subject: "Mess management app password reset",
      message: message,
    });
